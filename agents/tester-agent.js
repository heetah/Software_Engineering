...
export async function generateJestTests(sessionId, mapping, agent) {
  ...
  for (const item of mapping) {
    try {
      ...
      console.log(`\n[INFO] Processing ${item.jsFile}...`);
      console.log(`  - Test Strategy: ${analysis.testStrategy || 'unit'}`);
      ...
      if (!syntaxResult.valid && syntaxResult.wasFixed) {
        ...
        console.log(`  [SYNTAX] ✓ Automatically fixed syntax errors: ${fileName}`);
      } else if (!syntaxResult.valid) {
        // 無法修復，記錄錯誤但繼續
        console.warn(`  [SYNTAX] ✗ Syntax errors cannot be automatically fixed: ${fileName}`);
        console.warn(`    Errors: ${syntaxResult.errors.map(e => e.message).join('; ')}`);
      }
      ...
      console.log(`  [PATCH] Patched source code -> ${fileName}`);
      ...
      if (analysis.testStrategy === 'hybrid') {
        // 生成單元測試
        console.log(`[INFO] Generating unit test...`);
        ...
        console.log(`[SUCCESS] Unit test generated: ${basename}.unit.test.js`);

        // 生成整合測試
        console.log(`[INFO] Generating integration test...`);
        ...
        console.log(`[SUCCESS] Integration test generated: ${basename}.integration.test.js`);
        ...
        console.log(`[SUCCESS] Integration test generated: ${basename}.test.js`);
      } else {
        // 只生成單元測試
        const unitTestCode = await generateUnitTest(item, sourceCode, agent, sessionId, actualSourcePath, analysis);
        const testFilePath = path.join(testsDir, `${basename}.test.js`);
        await fs.promises.writeFile(testFilePath, unitTestCode, 'utf-8');
        console.log(`[SUCCESS] Unit test generated: ${basename}.test.js`);
      }
    } catch (err) {
      console.error(`[ERROR] Failed to generate Jest tests: ${item.jsFile} - ${err.message}`);
    }
  }
...
export function fixTestPaths(testCode, correctPath, basename) {
  ...
  if (fixedCount > 0) {
    console.log(`[PATCH] fixTestPaths: Fixed ${fixedCount} incorrect require paths`);
  }
...
export function validateAndFixSyntax(code, fileName = 'unknown.js') {
  ...
  // 第二階段：嘗試自動修復
  console.log(`[SYNTAX] ${fileName}: Discover syntax errors, trying to automatically fix...`);
  let fixedCode = code;
  let fixAttempts = [];
  ...
  if (expectedClosings > actualClosings) {
    ...
    fixAttempts.push(`Adding ${missing} missing '});'`);
  }
  ...
  if (lastLine && !lastLine.endsWith(';') && !lastLine.endsWith('{') && !lastLine.endsWith('}') && !lastLine.endsWith(',')) {
    if (/^(?:const|let|var|return|throw|export|import)\s/.test(lastLine.trim()) || /\)\s*$/.test(lastLine)) {
      fixedCode = fixedCode.trimEnd() + ';';
      fixAttempts.push('Adding missing semicolon');
    }
  }
  ...
  if (trailingCommaFix !== fixedCode) {
    ...
    fixAttempts.push('Remove trailing comma');
  }
  ...
  if (doubleSemicolonFix !== fixedCode) {
    ...
    fixAttempts.push('Remove duplicate semicolons');
  }
  ...
  const fixedLines = lines.map((line, idx) => {
    ...
    if (singleQuotes % 2 !== 0) {
      fixAttempts.push(`Line ${idx + 1}: Fixed unclosed single quotes`);
      return line + "'";
    }
    // 檢查雙引號
    const doubleQuotes = (line.match(/(?<!\\)"/g) || []).length;
    if (doubleQuotes % 2 !== 0) {
      fixAttempts.push(`Line ${idx + 1}: Fixed unclosed double quotes`);
      return line + '"';
    }
    return line;
  });
  ...
  try {
    ...
    console.log(`[SYNTAX] ${fileName}: ✓ Automatically fixed syntax errors (${fixAttempts.join(', ')})`);
    return result;
  } catch (secondError) {
    result.valid = false;
    result.errors.push({
    ...
    console.log(`[SYNTAX] ${fileName}: ✗ Automatically fix failed - ${secondError.message}`);
    return result;
  }
}
...
export function detectExportableFunctions(sourceCode) {
  ...
  console.log(`[DETECT] Exportable functions: ${result.functions.join(', ') || '(none)'}`);
  console.log(`[DETECT] Exportable variables: ${result.variables.join(', ') || '(none)'}`);
  console.log(`[DETECT] Existing exports: ${result.hasExistingExports ? 'yes' : 'no'}`);
...
export function generateExportsCode(exportInfo, options = {}) {
  ...
  if (exportInfo.hasExistingExports) {
    console.log(`[EXPORT] Already has exports, skipping generation`);
    return '';
  }
  ...
  if (toExport.length === 0) {
    console.log(`[EXPORT] No exportable items found`);
    return '';
  }
...
`;
  ...
  console.log(`[EXPORT] Generated exports: { ${toExport.join(', ')} }`);
  return exportsCode;
}
...
export async function ensureTestDependencies(sessionId, testCodes) {
  ...
  console.log(`\n[DEPS] Dependency analysis results:`);
  console.log(`  NPM packages: ${npmDeps.length > 0 ? npmDeps.join(', ') : 'none'}`);
  console.log(`  Polyfills: ${polyfills.length > 0 ? polyfills.join(', ') : 'none'}`);
  ...
  if (npmDeps.length === 0 && polyfills.length === 0) {
    console.log(`[DEPS] No additional dependencies to install`);
    return result;
  }
  ...
  if (npmDeps.length > 0) {
    ...
    if (missingDeps.length > 0) {
      console.log(`[DEPS] Installing missing dependencies: ${missingDeps.join(', ')}`);
      ...
      try {
        ...
        console.log(`[DEPS] ✓ Dependencies installed successfully: ${missingDeps.join(', ')}`);
      } catch (installError) {
        console.error(`[DEPS] ✗ Failed to install dependencies: ${installError.message}`);
        result.npmFailed = missingDeps;
      }
    } else {
      console.log(`[DEPS] All NPM dependencies already exist`);
    }
  }
...
`);
    ...
    if (polyfillsToAdd.length > 0) {
      ...
      console.log(`[DEPS] ✓ Added Polyfills: ${polyfills.join(', ')}`);
    }
  }
...
export async function preTestValidation(sessionId) {
  ...
  console.log(`\n[PRE-TEST] Starting pre-test validation for Session ${sessionId.substring(0, 8)}...`);
  ...
  try {
    ...
    if (!fs.existsSync(testsDir)) {
      result.errors.push({ type: 'directory', message: 'Test directory does not exist', path: testsDir });
      result.passed = false;
      return result;
    }
    ...
    if (testFiles.length === 0) {
      result.errors.push({ type: 'no-tests', message: 'No test files found' });
      result.passed = false;
      return result;
    }
    ...
    for (const testFile of testFiles) {
      ...
      try {
        ...
        result.errors.push({ type: 'read-error', file: testFile, message: `Failed to read file: ${readError.message}` });
        continue;
      }
    ...
    if (testCodes.length > 0) {
      ...
      if (depResult.npmFailed.length > 0) {
        result.warnings.push({
          type: 'deps-failed',
          message: `Some dependencies failed to install: ${depResult.npmFailed.join(', ')}`
        });
      }
    ...
    if (!fs.existsSync(jestConfigPath)) {
      result.warnings.push({ type: 'no-config', message: 'jest.config.cjs does not exist' });
    }
  ...
  // 輸出結果
  console.log(`\n[PRE-TEST] Pre-test validation results:`);
  console.log(`  Test files: ${result.summary.testFiles || 0} files`);
  console.log(`  Patched files: ${result.summary.patchedFiles || 0} files`);

  if (result.fixes.length > 0) {
    console.log(`  ✓ Automatic fixes: ${result.fixes.length} files`);
  ...
  if (result.warnings.length > 0) {
    console.log(`  Warnings: ${result.warnings.length} warnings`);
  ...
  if (result.passed) {
    console.log(`\n[PRE-TEST] Pre-test passed, ready to run tests\n`);
  } else {
    console.log(`\n[PRE-TEST] Pre-test failed, found ${result.errors.length} errors\n`);
  }
...
`;
      ...
      console.log(`[WARN] DOM extraction failed, keeping original code: ${extractError.message}`);
      // 如果提取失敗，保持原始碼不變
    }
  }
...
`;
    console.log(`[SUCCESS] Jest config set (hybrid: unit[node] + integration[jsdom])`);
...
`;
    console.log(`[SUCCESS] Jest config set (integration: jsdom)`);
  } else {
    // 純單元測試：使用 node
    jestConfig = `module.exports = {
  testEnvironment: 'node',
...
export async function runJestTests(sessionId) {
  ...
  if (!preCheckResult.passed) {
    console.log('[WARN] Pre-test found errors, still trying to run tests');
  ...
  console.log(`\n[INFO] Running Jest tests (${testFiles.length} test files)...`);
...
export async function writeTestReport(sessionId, jestResults, mapping) {
  ...
  if (!jestResults.results) {
    report += `## Status\n\n`;
    report += `⚠️  Jest execution failed or no test results\n\n`;
...
export async function runTesterAgent(sessionId) {
  ...
  try {
    console.log(`\n Starting test session: ${sessionId}`);
    ...
    // 2. 讀取所有測試計劃
    const testPlans = await loadTestPlans(sessionId);
    console.log(` ${testPlans.length} test plans loaded`);

    // 3. 建立映射
    const mapping = createJsTestPlanMapping(architectureData, testPlans);
    console.log(` ${mapping.length} JS files mapped to test plans`);

    // 4. 產生 Jest 測試
    const needsJSDOM = await generateJestTests(sessionId, mapping, agent);
    console.log(`Jest environment: ${needsJSDOM ? 'jsdom (DOM testing)' : 'node (pure logic testing)'}`);
    ...
    console.error(`Tester Agent failed: ${err.message}`);
    throw err;
  }
}
...
if (isMainModule()) {
  ...
  if (!sid) {
    console.error('Usage: node tester-agent.js <sessionId>');
    process.exit(1);
  }
