  ...
  async generate({ skeleton, fileSpec, context }) {
    ...
    try {
      if (context.allFiles && context.allFiles.length > 0) {
        ...
        const ContractsExtractor = require('../../coder-agent/contracts-extractor.js'); // User-requested enhancement
        ...
        // 2. Ingest known files (context.allFiles) - RE-ENABLED for better RAG
        ...
        // 3. ENHANCED INDEXING: Extract & Ingest Contracts - RE-ENABLED
        try {
          const extractor = new ContractsExtractor();
          const extracted = extractor.extractFromFiles(context.allFiles);

          let summary = `# Project Contracts Index\n\n`;

          if (extracted.dom.length > 0) {
            summary += `## DOM Elements\n`;
            extracted.dom.forEach(d => {
              summary += `- ID: ${d.id} (${d.type}) - ${d.purpose}\n`;
            });
            summary += `\n`;
          }

          if (extracted.api.length > 0) {
            summary += `## API Endpoints\n`;
            extracted.api.forEach(a => {
              summary += `- ${a.method} ${a.endpoint} - ${a.purpose}\n`;
            });
            summary += `\n`;
          }

          if (extracted.storage.length > 0) {
            summary += `## Storage Keys\n`;
            extracted.storage.forEach(s => {
              summary += `- ${s.key} (${s.type}) - ${s.purpose}\n`;
            });
          }

          // Ingest this high-value summary
          if (summary.length > 30) {
            await ragEngine.ingestFile('__contracts_summary__.md', summary);
            console.log('[Generator] 🧠 Ingested Contracts Summary for RAG');
          }
        } catch (extractErr) {
          console.warn('[Generator] Contracts extraction for RAG failed:', extractErr.message);
        }
