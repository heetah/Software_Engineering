  ...
  async _executeAPI(payload, retries = this.maxRetries) {
    ...
    if (apiProviderManager.providers.length > 0) {
      try {
        ...
        // 如果所有提供者都失敗，嘗試使用傳統方法（如果配置了單一 API）
      ...
      try {
        ...
        if (statusCode === 429) {
          ...
          if (retryAfter) {
            ...
            delay = Math.max(retryAfterSeconds * 1000, this.retryDelay);
            console.warn(`  ${this.role} Rate limited (429), API suggests waiting ${retryAfterSeconds} seconds before retry (${attempt + 1}/${retries})...`);
          } else {
            // 沒有 Retry-After 頭時，使用增強的指數退避
            // 對於 429 錯誤，使用更長的延遲時間
            delay = Math.max(this.retryDelay * Math.pow(2, attempt + 1), 2000);
            console.warn(`  ${this.role} Rate limited (429), retrying after ${delay}ms (${attempt + 1}/${retries})...`);
          }
